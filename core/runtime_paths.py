...
def resolve_memory_path(config: EviduneConfig, base_dir: Path) -> str:
    return resolve_runtime_path(config.memory.path, base_dir)


def resolve_emergence_output_dir(config: EviduneConfig, base_dir: Path) -> str:
    if not config.agent:
        return ""
    return resolve_runtime_path(config.agent.emergence.output_dir, base_dir)


def resolve_runtime_dir(config: EviduneConfig, base_dir: Path) -> str:
    if not config.agent:
        return ""
    return resolve_runtime_path(config.agent.harness.environment.runtime_dir, base_dir)


def resolve_metrics_config(config: EviduneConfig, base_dir: Path) -> dict[str, Any]:
